...
const PODCAST_PLATFORMS = {
	...
	podchaser: {
		name: 'Podchaser',
		path: 'M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.5 17.5h-11v-11h11v11zm-9-2h7v-7h-7v7zm2-5h3v3h-3v-3z'
	}
};
...
const registerPodcastSocialVariation = ( platformKey, platformConfig ) => {
	registerBlockVariation( 'core/social-link', {
		...
		icon: createPlatformIcon( platformConfig.path ),
		attributes: {
			service: platformKey,
		},
		isActive: [ 'service' ]
...
const initializePodcastSocialVariations = () => {
	Object.entries( PODCAST_PLATFORMS ).forEach( ( [ key, config ] ) => {
		registerPodcastSocialVariation( key, config );
	} );
};

// Initialize when script loads
initializePodcastSocialVariations();